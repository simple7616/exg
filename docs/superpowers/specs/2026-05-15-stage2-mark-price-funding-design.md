...
### 4.4.1 Triggered-conditional fill replay reconciliation (Task 9 — execution finding)

The Stage 2 C10 e2e (`replay_mark_price_trigger_survives_reboot`) surfaced a P1 recoverability gap that the original passive-replay design did not cover, found during Task 8 execution:

**Live model (verified):** a conditional order (`is_conditional()`) emits exactly ONE `OrderAccepted` with its conditional `order_type` and lands in `stop_orders` (engine.rs:140-163). When a `MarkPriceUpdate` triggers it, `trigger_and_match_stops` (engine.rs:777-819) `swap_remove`s it from `stop_orders`, converts the type (`StopMarket`/`TakeProfitMarket`/`TrailingStop` → `Market` with price `MAX`(Buy)/`ZERO`(Sell); `StopLimit`/`TakeProfitLimit` → `Limit`), runs the matcher, emits the **taker** `OrderFilled` carrying the conditional order's id — with **NO fresh `OrderAccepted`** — and reinserts leftover into the orderbook **only if** `!remaining_qty.is_zero() && order_type.is_limit()` (a converted Market never rests; its leftover is dropped).

**Replay gap:** non-conditional takers reach the orderbook via their pre-fill `OrderAccepted` so the existing `OrderFilled` arm resolves them; a triggered conditional is still in `stop_orders` (replay's `MarkPriceUpdate` arm is passive — invariant 27, no re-trigger) and was never promoted, so the `OrderFilled` arm's orderbook-only lookup returns `UnknownOrder` and **boot aborts** — the WAL is unreplayable once any mark-triggered stop fills. (The `OrderCanceled` arm already has the correct `stop_orders` fallback; `OrderFilled` was simply never extended for it.)

**Reconciliation rule (invariant 27 amended — replay stays matcher-free):** when the `OrderFilled` (and, where applicable, `TradeExecuted`/qty-bookkeeping) replay path encounters an `order_id` that is **not** in the orderbook but **is** in `stop_orders`, it performs the deterministic, matcher-free analogue of the live trigger: remove it from `stop_orders`, apply the conditional→`Market`/`Limit` type conversion exactly as `trigger_and_match_stops` does, and reconcile the fill so that **final replayed state is identical to live**: a fully-filled or `Market`-converted-with-leftover order ends removed (Market never rests — leftover dropped, mirroring live); a `Limit`-converted order with `remaining_qty > 0` ends resting in the orderbook with the correct remaining quantity so subsequent fills/cancels for that id resolve normally. This re-triggers nothing and runs no matcher (invariant 27 preserved) — the fill quantities come entirely from the WAL events.

**Correctness is defined by live↔replay state equivalence, not prose.** The exact multi-fill / partial-fill / leftover semantics MUST be pinned by round-trip equivalence tests (live engine processes the command stream; every emitted event is replayed into a fresh engine; assert identical orderbook order-count + per-order remaining quantities + `stop_orders` contents + `mark_price` + `last_funding_rate`) across at minimum: StopMarket single-maker full fill; StopMarket sweeping ≥2 makers; StopLimit partial → remainder rests → further fill; StopMarket insufficient-liquidity partial (leftover dropped); TrailingStop triggered. The implementation derives exact behavior from the live engine via TDD — it does not assume.

`Event` enum is still unchanged (spec §4.2 holds) — Stage 1b-era WALs remain replayable; the fix is purely in `replay.rs` resolution logic.
...
- **#27** `apply_event(MarkPriceUpdate)` is passive-only during replay — it never re-triggers stop/take-profit/trailing orders nor invokes the matcher. The triggered fills are independent WAL `OrderFilled` events. **(Amended, Task 9):** because the `MarkPriceUpdate` arm is passive, a triggered conditional order is still in `stop_orders` when its taker `OrderFilled` is replayed; the fill-resolution path therefore reconciles `order_id`s found in `stop_orders` (not the orderbook) by performing the matcher-free deterministic analogue of the live trigger (remove from `stop_orders`, apply the conditional→Market/Limit type conversion, drive replayed state to live-equivalent — see §4.4.1). No matcher runs and nothing is re-triggered; invariant 27's "passive, matcher-free" guarantee is preserved. Verified by live↔replay round-trip state-equivalence tests.
...
- **Path-3 replay discriminator masks post-terminal soft-corruption** (eng review): Task 9's `replay_seen_order_ids` Path-3 discriminator fail-fasts on never-accepted (orphan) `OrderFilled` ids — the primary recoverability guarantee: a corrupt / truncated / reordered WAL referencing a never-accepted order aborts boot rather than replaying into wrong state (tested by `boot_panics_on_unknown_order_filled`). But by design it treats ANY post-terminal `OrderFilled` for an already-seen id as a benign no-op regardless of `remaining_qty`. This is unavoidable at Stage 2: a legitimate multi-maker-insufficient-liquidity taker sweep and a hypothetical spurious duplicate fill of an already-consumed order are event-indistinguishable (both: seen id, absent from orderbook + stop_orders, carrying the same constant non-zero final remaining). A tighter discriminator (additionally requiring `remaining_qty.is_zero()`) is NOT viable — it would false-abort the legitimate insufficient-liquidity sweep, whose every taker leg carries the same constant non-zero leftover (locked by `replay_equiv_stop_market_multi_maker_insufficient_liquidity_nonzero_leftover` + the non-zero-remaining assertion block in `apply_terminal_duplicate_taker_leg_is_noop`). A robust fix needs a per-fill sequence / trade-id cross-check or aggregate-taker-fill emission (an `engine.rs` `emit_fill_events` change touching live WAL semantics + Stage 1b compatibility) — deferred until Stage 3+ widens the surface (liquidations / new order types).
